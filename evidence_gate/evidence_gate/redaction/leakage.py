...
PII_PATTERNS = [
    ...
    # Vietnamese MSISDN forms the redactor (jira_redactor._PATTERNS) masks but
    # this gate previously missed, letting raw `84974515324` / `0974515324`
    # pass plan-safety and report-review (the exact shapes that leaked in
    # TTSTK-3919 / XLSCVD-218). Kept in sync with jira_redactor. Length stays
    # under 13 digits to avoid colliding with Unix-ms timestamps.
    (re.compile(r"\b84\d{9,10}\b"), "raw phone number detected"),
    (re.compile(r"\b0\d{9,10}\b"), "raw phone number detected"),
    (re.compile(r"\b\d{3}-\d{2}-\d{4}\b"), "raw SSN detected"),
]

CREDENTIAL_PATTERNS = [
    (re.compile(r"(?i)\b(?:password|passwd|secret|token|api[_-]?key)\s*[:=]\s*\S+"), "credential assignment detected"),
